...
def perform_pnp_recon(points3D, features, cameras, matches, images):
    ...
    for i in tqdm(range(len(cameras_mod) - 2)):
        ...
        points3D, cameras_mod[: i + 2], colors3D = perform_BA(
            points3D_extended, cameras_mod[: i + 2], colors_extended
        )

    return points3D, colors3D, cameras_mod
...
def recon_all(
    ...
    if recon_method == "epipolar":
        ...
        # 执行PnP算法来估计相机姿态
        points3D_mod, colors3D_mod, cameras_mod = perform_pnp_recon(
            points3D, features, cameras, matches, images
        )
